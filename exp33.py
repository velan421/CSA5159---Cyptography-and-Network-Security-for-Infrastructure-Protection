# ============================================================
# EXPERIMENT 33 - DES (DATA ENCRYPTION STANDARD)
# ============================================================
...
def des_decrypt_block(block64, subkeys):
    return des_encrypt_block(block64, list(reversed(subkeys)))

def hex_to_bits64(h):
    val = int(h, 16)
    return [(val >> (63-i)) & 1 for i in range(64)]

def bits64_to_hex(bits):
    val = 0
    for b in bits:
        val = (val << 1) | b
...
key_hex = input("\nEnter 64-bit key in hex (e.g. 133457799BBCDFF1): ").strip()
pt_hex  = input("Enter 64-bit plaintext in hex (e.g. 0123456789ABCDEF): ").strip()

if not key_hex: key_hex = "133457799BBCDFF1"
if not pt_hex:  pt_hex  = "0123456789ABCDEF"

key_bits = hex_to_bits64(key_hex)
pt_bits  = hex_to_bits64(pt_hex)
...
ct_hex  = bits64_to_hex(ct_bits)
print(f"Ciphertext: {ct_hex}")

dt_bits = des_decrypt_block(ct_bits, subkeys)
dt_hex  = bits64_to_hex(dt_bits)
print(f"Decrypted : {dt_hex}")
print(f"Match     : {dt_hex == pt_hex.upper()}")
